...
export function createAssetMiddleware(getCurrentProjectId: () => string) {
  return async (c: { req: { path: string }; body: (data: Uint8Array, status: number, headers: Record<string, string>) => Response; text: (text: string, status: number) => Response }, _next: () => Promise<void>) => {
